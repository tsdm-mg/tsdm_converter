...
final class SeasonFinalsReport implements BaseReport {
  ...
  String toReport() {
    ...
    final repResultWithPromoteInfo = calculateBangumiPromoteCount(
      promoted: repechageBangumiResult,
      all: repechageBangumiResult,
    );

    final endingStateResultWithPromoteInfo = calculateBangumiPromoteCount(
      promoted: mergedBangumiResult,
      all: mergedBangumiResult,
    );
...
[tr][td=1,${finalsCharacters.length}][size=3]决赛[/size][/td]${finalsCharacters.map((e) => e.toBBCode()).join('\n[tr]')}
[tr=#fffacd][td=5,1]
[/td][/tr]
...
[tr][td=1,${repechageBangumiResult.length}][size=3]复活赛[/size][/td]${repResultWithPromoteInfo.map((e) => e.toReportOnlyPromotedCount()).join('\n[tr]')}
[tr=#fffacd][td=3,1]
[/td][/tr]
...
[tr][td=1,${mergedBangumiResult.length}][size=3]完结篇[/size][/td]${endingStateResultWithPromoteInfo.map((e) => e.toReportOnlyPromotedCount()).join('\n[tr]')}
[/table]
</pre>
