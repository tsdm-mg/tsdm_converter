...
/// 星耀复赛
final class StarlightRematchReport implements BaseReport {
  /// Constructor.
  const StarlightRematchReport({
  ...
  /// Build the report.
  factory StarlightRematchReport.build({
...
${promoteFinals.map((e) => '[td]${e.ranking}[/td][td]${e.name}@${e.bangumi}[/td][td][color=silver]${e.all}[/color][/td][td]${e.effective}[/td][td]四分之一决赛[/td][/tr]').join('\n[tr]')}
[tr]
...
''';
  ...
  String toReport() {
    ...
    return '''
<pre>
[align=center][b][size=5][color=#ff0000]星耀萌战2025复赛角色票数统计[/color][/size][/b][/align][table=98%]
...
[align=center][b][size=5][color=#ff0000]星耀萌战2025复赛晋级状况[/color][/size][/b][/align][table=98%]
...
[tr][td=1,${promotedBangumiResult.length}][size=3]四分之一决赛[/size][/td]${finalsResultWithPromoteInfo.sortByCharacters().map((e) => e.toReportOnlyPromotedCount()).join('\n[tr]')}
[/td][/tr]
[/table]
</pre>
