...
/// 星耀半决赛
final class StarlightSemiFinalsReport implements BaseReport {
  /// Constructor.
  const StarlightSemiFinalsReport({
  ...
  /// Build the report.
  factory StarlightSemiFinalsReport.build({
    ...
    final groupAPromotedRanking = groupAPoll.skip(promoteLimit).toList();
    final groupBPromotedRanking = groupBPoll.skip(promoteLimit).toList();

    final finalsBangumiResult =
        collectBangumiInfo([...groupAPromoted, ...groupBPromoted]);

    return StarlightSemiFinalsReport(
  ...
  String _generateGroupInfo(
    ...
    return '''
[tr][td=1,${promoteFinals.length + promoteRanking.length}][color=#4169e1]$groupName[/color][/td]
${promoteFinals.map((e) => '[td]${e.ranking}[/td][td]${e.name}@${e.bangumi}[/td][td][color=silver]${e.all}[/color][/td][td]${e.effective}[/td][td]决赛[/td][/tr]').join('\n[tr]')}
[tr]
${promoteRanking.map((e) => '[td]${e.ranking}[/td][td]${e.name}@${e.bangumi}[/td][td][color=silver]${e.all}[/color][/td][td]${e.effective}[/td][td]排位赛第二场[/td][/tr]').join('\n[tr]')}
''';
  }
  ...
  String toReport() {
    ...
    return '''
<pre>
[align=center][b][size=5][color=#ff0000]星耀萌战2025半决赛角色票数统计[/color][/size][/b][/align][table=98%]
...
${_generateGroupInfo(groupName: '第1组', promoteFinals: groupAPromoted, promoteRanking: groupAPromotedRanking)}
[tr=#fffacd][td=6,1][/td][/tr]
${_generateGroupInfo(groupName: '第2组', promoteFinals: groupBPromoted, promoteRanking: groupBPromotedRanking)}
[tr=#fffacd][td=6,1][/td][/tr]
[/table]

[align=center][b][size=5][color=#ff0000]星耀萌战2025半决赛晋级状况[/color][/size][/b][/align][table=98%]
